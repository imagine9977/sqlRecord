				...
				<div id="files" class="file-list">
					<c:if test="${not empty notice.files}">
						<c:forEach var="file" items="${notice.files}">
							<div>
								<span>${fn:escapeXml(file.nfileNo)}. </span> <a
									href="${path0}/${file.changedName}"
									download="${file.originalName}">
									${fn:escapeXml(file.originalName)} </a> <input type="file"
									class="form-control-file border" name="updatefile" />
								<button type="button" class="fileDelBtn"
									data-nfileNo="${file.nfileNo}">삭제</button>
							</div>
						</c:forEach>
